 ...
 *
 * These mimic RtlCompressBuffer and RtlDecompressBuffer from NTDLL.DLL. They
 * provide access to LZNT1, Xpress (LZ), and Xpress Huffman algorithms. They
 * attempt to provide results similar to the RTL functions in terms of speed and
 * compression rate. The provided functions always use the 'maximum engine'. For
 * differences in the specific algorithms see their header files.
 *
 * Additionally, there are streaming versions of the compressors and
 * decompressors.
 *
 * These functions are simple wrappers around the functions available in the
 * other header files.
 *
 * @author <a href="mailto:vavivavi@yahoo.co.jp">Naohide Sano</a> (nsano)
 * @version 0.00 2019/10/15 nsano initial version <br>
 */
public interface MSCompression extends Library {
     ...
     * given format.
     * <p>
     * This is essentially equivalent to the NTDLL function RtlCompressBuffer. To
     * convert:
     *
     * <pre>
     *  // chunk size and temporary buffer arguments are dropped
     *  err = RtlCompressBuffer(FORMAT|MAX, in, in_len, out, out_len, *, &out_len, *);
     *  // out_len is both in and out
     *  err = ms_compress(FORMAT, in, in_len, out, &out_len);
     * </pre>
     * 
     ...
     * given format.
     * <p>
     * This is essentially equivalent to the NTDLL function RtlDecompressBuffer. To
     * convert:
     *
     * <pre>
     *  // switched order of in and out!
     *  err = RtlDecompressBuffer(FORMAT, out, out_len, in, in_len, &out_len);
     *  // out_len is both in and out
     *  err = ms_decompress(FORMAT, in, in_len, out, &out_len);
     * </pre>
     *
     * @param format one of MSCOMP_NONE (0), MSCOMP_LZNT1 (2), MSCOMP_XPRESS (3), or
     *                   MSCOMP_XPRESS_HUFF (4) which are numerically equivalent to
