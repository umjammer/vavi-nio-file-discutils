...
public class BufferedFileSectorDevice extends AbstractFileSectorDevice {
    ...
    private static final Logger logger = getLogger(BufferedFileSectorDevice.class.getName());

    protected RandomAccessFile tocFile;
    protected ISectorDevice sectorDevice;
    protected Map<Integer, Integer> toc;
    ...
    protected void readToc() {
        ...
        try {
            ...
            logger.log(Level.ERROR, "readToc", e);
        }
    }
        ...
        if (tocDirty) {
            try {
                ...
                for (Integer sectorNumber : toc.keySet()) {
                    ...
                    tocFile.writeInt(sectorNumber);
                    tocFile.writeInt(bufferedSectorNumber);
                }
                tocDirty = false;
            } catch (IOException e) {
                logger.log(Level.ERROR, "writeToc", e);
            }
        }
    }
    ...
    public void readSector(int sectorNumber, byte[] buffer, int offset) throws IOException {
        ...
        if (bufferedSectorNumber != null) {
            fileAccess.seek(((long) sectorLength) * bufferedSectorNumber);
            fileAccess.read(buffer, offset, sectorLength);
            return;
        }

        if (sectorDevice == null) {
            logger.log(Level.WARNING, "Reading outside the UMD buffer file (sector=0x%X)".formatted(sectorNumber));
