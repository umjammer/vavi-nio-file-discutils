...
public class CHDFileSectorDevice extends AbstractFileSectorDevice {
    ...
    private static final Logger logger = getLogger(CHDFileSectorDevice.class.getName());
    ...
    public CHDFileSectorDevice(RandomAccessFile fileAccess, String fileName) throws IOException {
        ...
        if (err != CHDERR_NONE) {
            throw new IOException("Invalid CHD file '%s': %s".formatted(fileName, err));
        }
        chdFile = res[0];
        ...
        if (err == CHDERR_NONE) {
            ...
            if (m.find()) {
                ...
                logger.log(Level.INFO, "Track %d, type %s, subtype %s, frames %d, pregap %d".formatted(track, type, subtype, frames, pregap));

                numberFrames = frames;
                if ("MODE1".equals(type)) {
                    frameSize = sectorLength;
                } else {
                    throw new IOException("Unsupported CHD file format METADATA2='%s'".formatted(metadataString));
                }
            } else {
                throw new IOException("Unsupported CHD file format METADATA2='%s'".formatted(metadataString));
            }
        } else {
            throw new IOException("Unsupported CHD file format, could not find METADATA2 tag");
        }

        // The current readSector() implementation is only supporting a frameSize of 2048 bytes
        if (frameSize != sectorLength) {
            throw new IOException("Unsupported CHD file format having frameSize=%d, only %d is currently supported".formatted(frameSize, sectorLength));
        }
    ...
    private ChdError openChdFile(String fileName, ChdFile[] res) throws IOException {
        // First try to open without giving a parent
        ChdError err = chd.chd_open_file(fileName, FileAccess.Read, null, res);
            ...
            if (parentFileNames != null) {
                ...
                for (int i = 0; i < parentFileNames.length; i++) {
                    ...
                    if (err == CHDERR_NONE) {
                        ...
                        err = chd.chd_open_file(fileName, FileAccess.Read, parent, res);
                        if (err == CHDERR_NONE) {
    ...
    public void readSector(int sectorNumber, byte[] data, int offset) throws IOException {
        ...
        if (hunkNumberInBuffer != hunkNumber) {
            ...
            if (err != CHDERR_NONE) {
                throw new IOException("Error reading CHD file sectorNumber=%d: %s".formatted(sectorNumber, err));
            }
